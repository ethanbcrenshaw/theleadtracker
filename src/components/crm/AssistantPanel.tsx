...
export function AssistantPanel({ open, onClose }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { data } = await sb.from("assistant_messages").select("*").order("created_at", { ascending: true }).limit(200);
      if (cancelled || !data) return;
      const restored: Msg[] = data.map((r: { id: string; role: string; content: string; tool_calls: Step[] | null; pending_action: PendingAction | null }) => ({
        id: r.id, role: r.role as "user" | "assistant",
        content: r.content, steps: r.tool_calls ?? undefined, pending: r.pending_action ?? null,
      }));
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((cur) => [...cur, { id: crypto.randomUUID(), role: "assistant", content: j.reply || "", steps: j.steps, pending: j.pendingAction ?? null }]);
      void refresh();
    } catch (e) {
  ...
  async function executeConfirm(msgId: string, action: PendingAction, typedConfirmation?: string) {
    ...
    try {
      ...
      const j = await res.json();
      if (!res.ok) throw new Error(j.error || `status ${res.status}`);
      setMessages((cur) => cur.map((m) => m.id === msgId ? { ...m, pending: null, executed: { ok: true, count: j.count, kind: j.kind } } : m));
      // Add a system-like assistant confirmation message.
      const confirmText = action.kind === "delete"
      ...
      setMessages((cur) => [...cur, { id: crypto.randomUUID(), role: "assistant", content: confirmText }]);
  ...
  function cancelPending(msgId: string) {
    setMessages((cur) => cur.map((m) => m.id === msgId ? { ...m, pending: null, executed: { ok: false, count: 0, kind: "cancelled" } } : m));
  }
      ...
      {open && (
        <>
          <motion.div
            ...
            className="fixed inset-0 bg-background/40 z-40 md:hidden" onClick={onClose}
          />
          <motion.aside
          ...
          >
            <header className="flex items-center justify-between px-5 py-4 border-b border-border">
              ...
              <button onClick={onClose} aria-label="Close" className="p-1 hover:bg-foreground/10"><X className="h-4 w-4" /></button>
            </header>
                ...
                <div key={m.id} className="space-y-2">
                  {m.role === "user" ? (
                    ...
                    <div className="space-y-2">
                      ...
                      {m.pending && (
                        <ConfirmationCard pending={m.pending} onConfirm={(t) => executeConfirm(m.id, m.pending!, t)} onCancel={() => cancelPending(m.id)} busy={busy} />
                      )}
                      {m.executed && (
                        <div className="mono text-muted-foreground">
                          {m.executed.kind === "cancelled" ? "— cancelled —" : `— ${m.executed.kind} confirmed —`}
                        </div>
                      )}
                    </div>
                  )}
  ...
  return (
    <div className="border border-foreground p-4 space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        >[ CONFIRM ]</button>
        <button onClick={onCancel} disabled={busy} className="mono ink-link">[ CANCEL ]</button>
      </div>
    </div>
  );
}