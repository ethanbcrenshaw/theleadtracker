...
function classify(websiteUri: string | undefined) {
  ...
  if (!websiteUri) return { opp: "No Dedicated Website", presence: "No website listed on Google", website: null, sources };
  const host = hostFromUrl(websiteUri);
  if (!host) return { opp: "No Dedicated Website", presence: "No usable website found", website: null, sources };
  for (const social of Object.keys(SOCIAL_HOSTS)) {
    if (host === social || host.endsWith("." + social)) {
      ...
      return { opp: SOCIAL_HOSTS[social], presence: `Uses a ${label} page instead of a website`, website: null, sources: [...sources, src] };
    }
  }
  if (isOneOf(host, DIRECTORY_HOSTS)) {
    return { opp: "Yelp/Directory Only", presence: `Listed on ${host} — no dedicated website`, website: null, sources: [...sources, "Directory"] };
  }
  return { opp: "Has Website", presence: `Has a website (${host})`, website: host, sources: [...sources, "Website"] };
}
...
async function placesSearch(query: string, apiKey: string, pageToken?: string): Promise<PlacesResponse> {
  const res = await fetch(PLACES_SEARCH, {
    ...
    body: JSON.stringify({ textQuery: query, maxResultCount: 20, ...(pageToken ? { pageToken } : {}) }),
  });
...
export async function discoverCandidates(opts: {
  industry: string; city: string; count: number; type: string; apiKey: string;
